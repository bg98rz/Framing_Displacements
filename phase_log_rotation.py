...
base_img = cv2.imread('184-C-3201905251028114.jpg')    # read image from path - edit this parameter to point to an image
base_img = np.float32(
  cv2.cvtColor(base_img, cv2.COLOR_BGR2GRAY)) / 255.0    # convert loaded image to necessary format
...
resized = cv2.resize(base_img, dim, 
                     interpolation = cv2.INTER_AREA)    # resize output window 
resized2 = cv2.resize(curr_img, dim, 
                      interpolation = cv2.INTER_AREA) 

cv2.imshow("base_img", resized)    # show original image and resized image
cv2.imshow("curr_img", resized2)

base_polar = cv2.linearPolar(base_img,(cX, cY),   # generate log-polar versions of original and rotated image
                             min(cX, cY), 0)    
curr_polar = cv2.linearPolar(curr_img,(cX, cY), 
                             min(cX, cY), 0) 

resizedp = cv2.resize(base_polar, dim, 
                      interpolation = cv2.INTER_AREA)
resizedp2 = cv2.resize(curr_polar, dim, 
                       interpolation = cv2.INTER_AREA) 
